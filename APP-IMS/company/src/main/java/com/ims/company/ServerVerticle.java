...
import com.ims.company.util.SqlQueries;
...
public class ServerVerticle extends AppRestAPIBaseVerticle {
	...
	public void start() throws Exception {
		...
		router.post("/api/ims/" + AddressConstants.INSERT_COMPANY).handler(routingContext -> {
				...
				JsonObject data = routingContext.getBodyAsJson();
						...
						processDbEbRequest(routingContext,
								PreProcessor.doProcess(data,
										PreProcessBuilder.getPreProcessList(AddressConstants.INSERT_COMPANY)),
								AddressConstants.INSERT_COMPANY, SqlQueries.INSERT_COMPANY);
				 
			} catch (Exception e) {
				processException(e, routingContext, AddressConstants.INSERT_COMPANY);
			}
		});

		router.get("/api/ims/" + AddressConstants.GET_COMPANY).handler(routingContext -> {

			try {
				processDbEbRequest(routingContext,
						PreProcessor.doProcess(new JsonObject(),
								PreProcessBuilder.getPreProcessList(AddressConstants.GET_COMPANY)),
						AddressConstants.GET_COMPANY, SqlQueries.GET_COMPANY);
			} catch (Exception e) {
				processException(e, routingContext, AddressConstants.GET_COMPANY);
			}
		});

		router.put("/api/ims/" + AddressConstants.UPDATE_COMPANY).handler(routingContext -> {
			try {
				JsonObject data = routingContext.getBodyAsJson();
						...
						processDbEbRequest(routingContext,
								PreProcessor.doProcess(data,
										PreProcessBuilder.getPreProcessList(AddressConstants.UPDATE_COMPANY)),
								AddressConstants.UPDATE_COMPANY, SqlQueries.UPDATE_COMPANY);
		 
			} catch (Exception e) {
				processException(e, routingContext, AddressConstants.UPDATE_COMPANY);
			}
		});

		router.delete("/api/ims/" + AddressConstants.DELETE_COMPANY).handler(routingContext -> {

			JsonObject data = new JsonObject();
			try {
				processDbEbRequest(routingContext,
						PreProcessor.doProcess(data,
								PreProcessBuilder.getPreProcessList(AddressConstants.DELETE_COMPANY)),
						AddressConstants.DELETE_COMPANY, SqlQueries.DELETE_COMPANY);
			} catch (Exception e) {
				processException(e, routingContext, AddressConstants.DELETE_COMPANY);
			}
		});
