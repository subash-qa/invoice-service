...
import com.ims.customer.util.SqlQueries;
...
public class ServerVerticle extends AppRestAPIBaseVerticle {
	...
	public void start() throws Exception {
		...
		router.post("/api/ims/" + AddressConstants.INSERT_CUSTOMER).handler(routingContext -> {
				...
				JsonObject data = routingContext.getBodyAsJson();
						...
						processDbEbRequest(routingContext,
								PreProcessor.doProcess(data,
										PreProcessBuilder.getPreProcessList(AddressConstants.INSERT_CUSTOMER)),
								AddressConstants.INSERT_CUSTOMER, SqlQueries.INSERT_CUSTOMER);
				 
			} catch (Exception e) {
				processException(e, routingContext, AddressConstants.INSERT_CUSTOMER);
			}
		});

		router.get("/api/ims/" + AddressConstants.GET_CUSTOMER).handler(routingContext -> {

			try {
				processDbEbRequest(routingContext,
						PreProcessor.doProcess(new JsonObject(),
								PreProcessBuilder.getPreProcessList(AddressConstants.GET_CUSTOMER)),
						AddressConstants.GET_CUSTOMER, SqlQueries.GET_CUSTOMER);
			} catch (Exception e) {
				processException(e, routingContext, AddressConstants.GET_CUSTOMER);
			}
		});

		router.put("/api/ims/" + AddressConstants.UPDATE_CUSTOMER).handler(routingContext -> {
			try {
				JsonObject data = routingContext.getBodyAsJson();
						...
						processDbEbRequest(routingContext,
								PreProcessor.doProcess(data,
										PreProcessBuilder.getPreProcessList(AddressConstants.UPDATE_CUSTOMER)),
								AddressConstants.UPDATE_CUSTOMER, SqlQueries.UPDATE_CUSTOMER);
			} catch (Exception e) {
				processException(e, routingContext, AddressConstants.UPDATE_CUSTOMER);
			}
		});
...
//		int portNo = Integer.parseInt(config().getString(ConfigConstants.HTTP_PORT_NO_KEY, "8082"));
	...
	public void mediaUpload(JsonObject jsonObject, Handler<AsyncResult<JsonObject>> mediaHandler) {
		...
		if (jsonObject != null) {
			processWebClientRequest(jsonObject, 8085, "localhost", "/api/ims/insert_media", result -> {
